...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    e.preventDefault();
    deferredPrompt = e;
    // Banner is always visible, just enable the button
...
function calculateAll() {
    ...
    for (let c = 1; c <= 5; c++) {
        ...
        setText('t' + c + '_t', boys + girls); setText('t' + c + '_ti', boysITN + girlsITN);
    ...
    setText('sum_total_boys', totalBoys); setText('sum_total_girls', totalGirls);
    setText('sum_total_pupils', totalPupils);
    setText('sum_boys_itn', totalBoysITN); setText('sum_girls_itn', totalGirlsITN);
    ...
    setText('prop_boys', propBoys + '%'); setText('prop_girls', propGirls + '%');
    setVal('prop_boys_val', propBoys); setVal('prop_girls_val', propGirls);
    ...
    setText('cov_boys', covBoys + '%'); setText('cov_girls', covGirls + '%');
    setText('cov_total', covTotal + '%');
    setVal('coverage_boys', covBoys); setVal('coverage_girls', covGirls);
...
function updateCharts() {
    ...
    if (ctx1) {
        ...
        state.charts.enrollment = new Chart(ctx1, {
            type: 'doughnut',
            data: { labels: ['Boys', 'Girls'], datasets: [{ data: [totalBoys, totalGirls], backgroundColor: ['#004080', '#e91e8c'], borderWidth: 2, borderColor: '#fff' }] },
    ...
    if (ctx2) {
        ...
        state.charts.itn = new Chart(ctx2, {
            type: 'doughnut',
            data: { labels: ['Boys', 'Girls'], datasets: [{ data: [boysITN, girlsITN], backgroundColor: ['#004080', '#e91e8c'], borderWidth: 2, borderColor: '#fff' }] },
            options: { responsive: true, maintainAspectRatio: true, plugins: { legend: { position: 'bottom' } } }
        });
    }
    ...
    if (ctx3) {
        ...
        state.charts.coverage = new Chart(ctx3, {
            type: 'bar',
            data: { labels: ['Class 1', 'Class 2', 'Class 3', 'Class 4', 'Class 5'], datasets: [{ label: 'Coverage %', data: coverages, backgroundColor: '#28a745', borderWidth: 0 }] },
...
function generateDraftName() {
    ...
    fields.forEach(id => {
        const el = document.getElementById(id);
        if (el && el.value) {
            let val = el.value.replace(' District', '').trim();
            parts.push(val);
        }
    });
    return parts.length === 0 ? 'Draft - ' + new Date().toLocaleDateString() : parts.join('-');
}
...
async function handleSubmit(e) {
    ...
    if (state.isOnline) {
        try {
            ...
            if (state.currentDraftId) { state.drafts = state.drafts.filter(d => d.draftId !== state.currentDraftId); saveToStorage(); updateCounts(); }
...
function openAnalysisModal() {
    ...
    Object.entries(districtData).forEach(([d, v]) => {
        districtRows += '<tr><td>' + d + '</td><td>' + v.schools + '</td><td>' + v.pupils.toLocaleString() + '</td><td>' + v.itn.toLocaleString() + '</td><td>' + (v.pupils > 0 ? Math.round((v.itn/v.pupils)*100) : 0) + '%</td></tr>';
    });

    body.innerHTML = '<div class="analysis-stats"><div class="stat-card"><div class="stat-value">' + totalSchools + '</div><div class="stat-label">Schools Surveyed</div></div><div class="stat-card"><div class="stat-value">' + totalPupils.toLocaleString() + '</div><div class="stat-label">Total Pupils</div></div><div class="stat-card"><div class="stat-value">' + totalITN.toLocaleString() + '</div><div class="stat-label">ITNs Distributed</div></div><div class="stat-card green"><div class="stat-value">' + coverage + '%</div><div class="stat-label">Overall Coverage</div></div></div><div class="analysis-section"><h3>Gender Breakdown</h3><div class="analysis-grid"><div class="analysis-item"><span class="item-label">Total Boys:</span><span class="item-value">' + totalBoys.toLocaleString() + '</span></div><div class="analysis-item"><span class="item-label">Total Girls:</span><span class="item-value">' + totalGirls.toLocaleString() + '</span></div><div class="analysis-item"><span class="item-label">Boys ITN:</span><span class="item-value">' + totalBoysITN.toLocaleString() + '</span></div><div class="analysis-item"><span class="item-label">Girls ITN:</span><span class="item-value">' + totalGirlsITN.toLocaleString() + '</span></div><div class="analysis-item"><span class="item-label">Boys Coverage:</span><span class="item-value">' + (totalBoys > 0 ? Math.round((totalBoysITN/totalBoys)*100) : 0) + '%</span></div><div class="analysis-item"><span class="item-label">Girls Coverage:</span><span class="item-value">' + (totalGirls > 0 ? Math.round((totalGirlsITN/totalGirls)*100) : 0) + '%</span></div></div></div><div class="analysis-section"><h3>By District</h3><table class="analysis-table"><thead><tr><th>District</th><th>Schools</th><th>Pupils</th><th>ITNs</th><th>Coverage</th></tr></thead><tbody>' + districtRows + '</tbody></table></div>';
    modal.classList.add('show');
}
...
async function syncPending() {
    ...
    for (let i = 0; i < state.pendingSubmissions.length; i++) {
        try {
            await fetch(CONFIG.SCRIPT_URL, { method: 'POST', mode: 'no-cors', body: JSON.stringify(state.pendingSubmissions[i]) });
            synced.push(i);
        } catch (e) {}
    }
